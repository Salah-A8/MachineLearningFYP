...
SVM_model = SVC(probability=True, class_weight='balanced') # Creating a SVM model with balanced class weights and probability estimators enabled
SVM_model.fit(x_train, y_train)# Training the Logistic Regression model on the training data
y_prediction = SVM_model.predict(x_test)# Letting the Logistic Regression model predict on the testing data
...
labels = SVM_model.classes_ # Gets the unique class labels from the trained model
y_binarised = label_binarize(y_test, classes=labels) # Converts class labels into binary for ROC curve
y_score = SVM_model.predict_proba(x_test) # Gets predicted probablites for each class
