...
decisiontree_model = DecisionTreeClassifier(class_weight='balanced') # Creating a Decision tree model with class weights balanced to handle class imbalances
decisiontree_model.fit(x_train, y_train) # Training the Decision Tree model on the training data
y_prediction = decisiontree_model.predict(x_test) # Letting the Decision Tree model predict on the testing data
...
labels = decisiontree_model.classes_ # Gets the unique class labels from the trained model
y_binarised = label_binarize(y_test, classes=labels) # Converts class labels into binary for ROC curve
y_score = decisiontree_model.predict_proba(x_test) # Gets predicted probablites for each class
