...
y = dataset['simplified_ratings'] # Extract target variable storing it in the y variabl
...
LR_model = LogisticRegression(max_iter=1000, class_weight='balanced') # Creating a Logistic Regression model with balanced class weights and a maximum of 1000 iterations
LR_model.fit(x_train, y_train) # Training the Logistic Regression model on the training data
y_prediction = LR_model.predict(x_test)# Letting the Logistic Regression model predict on the testing data
...
labels = LR_model.classes_ # Gets the unique class labels from the trained model
y_binarised = label_binarize(y_test, classes=labels) # Converts class labels into binary for ROC curve
y_score = LR_model.predict_proba(x_test) # Gets predicted probablites for each class
